...
public class Stage3BattleSystem : BaseBattleManager
{
    ...
    {
        //ドラゴンの鳴き声を実行
        EnemySE.Instance.Play_DragonRoarSE();
    ...
    ///ドラゴンの攻撃によるデバフ処理
    /// </summary>
    /// <param name="player">BasePlayerStatusを継承しているプレイヤーステータス</param>
    /// <param name="debuffHPTextkey">JsonテキストデータのデバフテキストID</param>
    /// <param name="token">キャンセルできるトークン</param>
    /// <returns></returns>
    async UniTask HPDebuff(BasePlayerStatus player, string debuffHPTextkey, CancellationToken token)
    {
        ...
        //プレイヤーのHPをドラゴンのデバフ(HPを減らすデバフ)の数値分体力を減らす
        ...
        //もしデバフカウントが0になったらJsonテキストIDからデバフ解除をUIで表示
