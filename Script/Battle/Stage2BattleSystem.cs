...
public class Stage2BattleSystem : BaseBattleManager
{
    ...
    async UniTask EnemyTurn(CancellationToken token)
    {
        ...
        {
            //スケルトン1ターン
            await SkeletonTurn(skeleton1, token);
            if (GameOverCheck()) return;
        }
        if (skeleton2.EnemyIsAlive)
        {
            //スケルトン2ターン
            await SkeletonTurn(skeleton2, token);
            if (GameOverCheck()) return;
        }
        ...
        {
            //スケルトン3ターン
            await SkeletonTurn(skeleton3, token);
            if (GameOverCheck()) return;
        }
    ...
    /// <param name="token">キャンセルできる処理</param>
    async UniTask SkeletonTurn(Skeleton skeleton, CancellationToken token)
    {
        //スケルトンアクション
