...
public class Healer : BasePlayerStatus
{
    ...
    private GameObject healer_HealEffect;

    [SerializeField]
    [Tooltip("アタッカーの回復エフェクトを表示させる位置")]
    private Transform attacker_HealEffect_SpawnPoint;

    [SerializeField]
    [Tooltip("バッファーの回復エフェクトを表示させる位置")]
    private Transform buffer_HealEffect_SpawnPoint;

    [SerializeField]
    [Tooltip("ヒーラーの回復エフェクトを表示させる位置")]
    private Transform healer_HealEffect_SpawnPoint;
    ...
    /// <summary>
    /// Josnデータがある時に読み込むメソッド
    /// </summary>
    /// <param name="data">Jsonの保存データ</param>
    ...
    public override void NormalAttack()
    {
        ...
        if (target != null)
        {
            ...
            GameObject textEffectInstance = Instantiate(healer_AttackTextEffect, PlayerTextEffect_SpawnPoint.position, Quaternion.identity);
    ...
    { 
        ...
        if (attacker.IsAlive)
        {
            specialEffects.Add(Instantiate(healer_HealEffect, attacker_HealEffect_SpawnPoint.position, Quaternion.identity));
        }

        if (buffer.IsAlive)
        {
            specialEffects.Add(Instantiate(healer_HealEffect, buffer_HealEffect_SpawnPoint.position, Quaternion.identity));
        }

        if (IsAlive)
        {
            specialEffects.Add(Instantiate(healer_HealEffect, healer_HealEffect_SpawnPoint.position, Quaternion.identity));
        }
    ...
    public void OnHeal(BasePlayerStatus target)
    {
        ...
        GameObject specialEffectInstance = Instantiate(healer_HealEffect, target.transform.position, Quaternion.identity);
        Destroy(specialEffectInstance, 3f);

        Debug.Log(target + "のHP" + target.PlayerCurrentHP);
